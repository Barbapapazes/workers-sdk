...
/**
 * Get bindings from config in the flat StartDevWorkerInput["bindings"] format.
 * Each binding is keyed by its binding name with a `type` discriminator.
 */
...
async function collectPendingResourcesFromBindings(
	...
	updatedBindings: StartDevWorkerInput["bindings"];
}> {
...
export async function provisionBindingsFromInput(
	...
	config: Config,
	requireRemote = false
): Promise<StartDevWorkerInput["bindings"]> {
